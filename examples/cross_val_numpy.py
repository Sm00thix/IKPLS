...
def cv_splitter(splits: npt.NDArray): # Splits is a 1D array of integers indicating the split number for each sample.
...
if __name__ == "__main__":
    ...
    splits = np.random.randint(0, 10, size=N) # Randomly assign each sample to one of 10 splits.

    # Using float64 is important for numerical stability.
    X = np.random.uniform(size=(N, K)).astype(np.float64)
    Y = np.random.uniform(size=(N, M)).astype(np.float64)

    np_pls_alg_1 = PLSWithPreprocessing(algorithm=1)
    fit_params = {"A": A}

...
# Summary
The `ikpls` software package introduces fast and versatile implementations of both versions of Improved Kernel Partial Least Squares (IKPLS) [@dayal1997improved]. `ikpls` also contains the fast cross-validation algorithm presented by Engstrøm [@engstrøm2024shortcutting] combined with both IKPLS algorithms.
...
Benchmarks demonstrate the superior performance of `ikpls` compared to scikit-learn's NIPALS across various scenarios. Additionally, the software addresses the redundant structure in cross-validation, proposing an algorithmic improvement for substantial speedup without recomputing total matrix products.
